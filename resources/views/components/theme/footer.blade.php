<footer
    class="w-full mt-auto bg-gray-50 dark:bg-transparent dark:bg-gradient-to-b dark:from-white/[0.03] dark:to-transparent">
    <div class="px-6 bg-gray-100">
        @livewire('front.newsletters')
    </div>
    <div class="px-6">
        <div class="grid gap-y-10 gap-x-4 py-14 sm:grid-cols-3 lg:grid-cols-4 lg:py-[100px]">
            <div class="relative">
                <img src="{{ asset('images/' . Helpers::settings('site_logo')) }}" loading="lazy"
                    alt="{{ Helpers::settings('site_name') }}" class="h-7">
                <ul class="my-6 flex flex-col gap-3 font-bold">
                    ...
                    <li>
                        <a href="tel:+(617) 254-2333"
                            class="inline-block transition hover:scale-110 hover:text-indigo-500">{{ Helpers::settings('company_phone') }}</a>
                    </li>
                </ul>
            ...
            <div>
                <ul class="flex flex-col gap-3 font-bold">
                    ...
                    <li><a href="{{ route('front.index') }}"
                            class="inline-block transition hover:scale-110 hover:text-indigo-500">{{ __('Home') }}</a>
                    </li>
                    <li><a href="{{ route('front.catalog') }}"
                            class="inline-block transition hover:scale-110 hover:text-indigo-500">{{ __('Catalog') }}</a>
                    </li>
                    <li><a href="{{ route('front.services') }}"
                            class="inline-block transition hover:scale-110 hover:text-indigo-500">{{ __('Service') }}</a>
                    </li>
                    <li><a href="#"
                            class="inline-block transition hover:scale-110 hover:text-indigo-500">{{ __('About Us') }}</a>
                    </li>
                </ul>
            </div>
            <div>
                    ...
                    @foreach (\App\Helpers::getActiveCategories() as $category)
                        <li>
                            <a href="{{ route('front.categories') }}"
                                class="inline-block transition hover:scale-110 hover:text-indigo-500">{{ $category->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
    ...
    <div class="bg-gray-100 py-5 dark:border-t-2 dark:border-white/5 dark:bg-none">
        <div class="px-6">
            <div class="flex flex-col items-center justify-between text-center font-bold dark:text-white md:flex-row">
                <div>
                    ...
                    <a href="javascript:" class="text-indigo-900 transition hover:text-indigo-500">
                        {{ Helpers::settings('company_name') }}
                    </a>
                </div>
                <div>{{ __('Need help? Visit the') }} <a href="{{ route('front.contact') }}"
                        class="text-indigo-500 transition hover:text-indigo-900">{{ __('Contact Us') }}</a></div>
            </div>
        </div>
    </div>
</footer>

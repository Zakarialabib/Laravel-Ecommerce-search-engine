<div>
    ...
    <x-table>
        ...
        <x-table.tbody>
            @forelse($device_models as $device_model)
                <x-table.tr wire:loading.class.delay="opacity-50" wire:key="row-{{ $device_model->id }}">
                    <x-table.td>
                        <input type="checkbox" value="{{ $device_model->id }}" wire:model="selected">
                    </x-table.td>
                    <x-table.td>
                        {{ $device_model->name }}
                    </x-table.td>
                    ...
                    <x-table.td>
                        <div class="flex justify-start space-x-2">
                            <x-button primary type="button" wire:click="$emit('editModal', {{ $device_model->id }})"
                                wire:loading.attr="disabled">
                                <i class="fas fa-edit"></i>
                            </x-button>
                            <x-button danger type="button" wire:click="deleteModal({{ $device_model->id }})"
                                wire:loading.attr="disabled">
                                <i class="fas fa-trash-alt"></i>
                            </x-button>
                        </div>
    ...
    <!-- Create Modal -->
    @livewire('admin.device-models.create')

    <!-- Edit Modal -->
    @livewire('admin.device-models.edit', ['device_model' => $device_model])
